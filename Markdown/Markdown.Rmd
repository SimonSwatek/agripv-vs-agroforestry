...
> This type of AFS falls within the broad definition of silvoarable AFS or "alley cropping systems". The tree strips consist of multiple rows of trees, that are densely planted (10.000 – 15.000 trees/ha) forming a dense hedgerow-like structure, that is regularly coppiced. The width of the tree strips can range from 3-25 m and can legally occupy 2-35 % of the arable field to be registered as an agroforestry system and be eligible to funding (Böhm, 2022)(see Fig. 1).In ground-mounted Agri-PV systems, typically only the areas between the PV modules are farmed (see figure 1). The advantages of ground-mounted systems are primarily their lower cost and low impact on the landscape. Vertically arranged modules in ground-mounted systems typically have maximum electricity production in the morning and evening hours.



```{r , fig.align = 'center', echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap="Fig.1: Dirk Freese 2014; in Quinkenstein et al., 2017"}
knitr::include_graphics('the_real_KUP.jpg')
```
...
![Fig. 2: Agrophotovoltaic system, ÖkoHaus GmbH](./agripv.png)
...
```{r,echo=TRUE, message=FALSE, warning=FALSE, fig.cap="Fig. 3: Location"}
library(chillR)
library(leaflet)
library(dplyr)
library(reshape2)
...
The first part includes the direct investment costs, the running costs and the profit of both decision options (see Fig. 4).
The actual conceptual model is illustrated in Fig. 4.
...
![Fig. 4: Conceptual Model](./conceptual_model.png)
...
```{r, warning=FALSE, fig.cap="Fig. 5: Input Data as estimates"} 
library(kableExtra)
df <- read.csv2("input_estimates_project_Sources.csv")
   kable(df) %>%    
   kable_styling("striped", position = "left", font_size = 10)  
```
  
> > The important variables from (Fig. 8) <br> "Pv annual costs"     [€/ha]: The annual running costs of the power installation <br>
...
```{r , fig.align = 'center', echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap="Fig.6: Comparision of differend NPVs"}
knitr::include_graphics('mc_simulation_3_decision.png')
```

> Monte-Carlo simulation of for the three possible decisions Agroforestry, Photovoltaic or Crop Rotation (Fig. 6). The probabilities for possible revenue or loss are graphed with this density histograms for intuitive comparison, ultimately guiding the decision process with figures that are as scientific as possible. <br>The x-axis shows outcome distribution in €/ha over the time span n_years = 26. <br>
...
```{r , fig.align = 'center', echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap="Fig. 7: Closer look into NPVs of Agroforestry and Crop Rotation"}
knitr::include_graphics('mc_simulation_af_crop_decision.png')
...
```{r , fig.align = 'center', echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap="Fig. 8: Closer look into NPVs of Photovoltaics and Crop Rotation" }
knitr::include_graphics('mc_simulation_pv_crop_decision.png')

```

> The exact same Monte-Carlo simulation, but without the values for Agroforestry to better compare Photovoltaics NPVs with Crop Rotation NPVs. This allows a better grasp of the Photovoltaics distribution. Its peak seems to be at roughly 20,000€/ha. The Photovoltaics distribution is split into two sub-distributions. One with a smaller sample size of values in the negative realm of the outcome distribution scale, its boundaries are -700,000€/ha for the lower end and at -400,000/ha at the upper end (Fig. 3) and one with the majority of values in the positive range of outcome distributions with the lower boundary at 0€/ha and the upper boundary at 40,000€/ha. The values for Crop Rotation are the exact same as described in (Fig. 7).

## 07.2 Boxplot-Densitiy
```{r , fig.align = 'center', echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap="Fig. 9: Agroforestry NPV histogram with boxplot"}
knitr::include_graphics('density_af.png')

```

> The stand alone distribution for Agroforestry like in (Fig. 6,7,8). The Boxplot at the bottom is classifying the set of the values, highlighting the outliers and describing the distribution. The 25 percentile is roughly at 30,000€/ha; the median at roughly 36,000€/ha; its 75 percentile at 51,000€/ha. The maximum value is at roughly 136,000€/ha.


```{r , fig.align = 'center', echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap="Fig. 10: Photovoltaics NPV histogram with boxplot"}
knitr::include_graphics('density_pv.png')

```

> The stand alone distribution for Photovoltaics like in (Fig. 6,7,8). The Boxplot at the bottom is classifying the set of the values, highlighting the outliers and describing the distribution. The 25 percentile is roughly at 18,000€/ha; the median at roughly 20,000€/ha; its 75 percentile at 22,000€/ha. The maximum value is at roughly 40,000€/ha and the absolute lowest value is -950,000€/ha.

## 07.3 Variable Importance Projected
```{r , fig.align = 'center', echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap="Fig. 11: The most relevant variables projected as a barplot"}
...
## 07.4 Cashflow 
```{r , fig.align = 'center', echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap="Fig. 12: Cashflows of the decision options"}
knitr::include_graphics('g_cashflow.png')
...
> We examined Agri-PV or AFS as an extension of the production spectrum from a farm. In the project work we used decision analysis approaches to model the contribution margins of two potential energy production systems - Agri-PV and AFS - over a time horizon of 26 years, thus providing a decision support tool for farms. Decision outcomes were compared with crop rotation, whether a transition, thus a reduction of the area where crops could be cultivated, to the cultivation of energy equivalents is economically sensible. 

> Our simulation results suggest the transition to AFS, as its NPV distribution is slightly more revenueable, than the NPV distribution of the pure Crop Rotation and clearly more revenueable than the NPV distribution of the Agri-PV (Fig. 6). There is a low probabilty that the outcome of Agri-PV system generates a negative outcome distribution, which underlines that this could be risky decision, without the chance of being more revenueable (Fig. 10).

> These points are well conlcluded within the cashflow projection (Fig. 12). This figure forecasts a bigger profit for the transition to AFS compared to the normal Crop Rotation and to the Agri-PV. Also the big risk for Agri-PV is additionally underlnied with possible huge negative range. <br>
The most influencial variables are being projected via VIP (Fig. 11). These three variables are all linked with the Agri-PV system. If the Initial investment costs and the annual running costs of the Agri-PV system can be reduced, than the NPV density distribution for the Agri-PV will be significantly positvely impacted. A potencial improvement of the PV panel design should impact the power densisty variable in positive way, because of a more consistent quality standard of the panels.<br>

> Agri-PV systems are becoming increasingly attractive for agriculture, because they offer a way to keep domestic agriculture competitive with the international market and provide farmers with additional income. However, the selection of the right crop varieties as well as a well-thought systems design are needed to combine an efficient production of crops and solar energy on farm.
The presented investment costs are based on free-field PV systems and not on allegedly cheaper Agri-PV systems and might therefore be overestimated.
If a decision is to be made based on the project results, we would suggest to include a defined arable field size and more accurate cost estimations to provide a more comprehensive picture.
To paraphrase the energy consultant from the agricultural extension agency of North Rhine-Westphalia (Landwirtschaftskammer - LWK), currently only Agri-PV systems are worthwhile, which are built cheaply, otherwise the returns might be too low. To allow for the decentralized supply of local renewable energy, policymakers must create the framework conditions that make even smaller agri-PV systems economically viable and enable farmers to operate them themselves.


# 09 Outlook

> As mentioned above, in both systems we consider a complete hectare of the systems.
This is because each system has a range of possible sizes that must be specified by the farmer.
The exact relations between the arable and the energy production component (i.e. the tree strips or the solar panels) can vary and are to be specified by the farmer.  
This is no longer a consideration of the fictitious farmer we assumed in our project, but an specific situation.
Since the  widths of energy production component in both systems do not have to be identical, but only can be, we decided on this procedure.

> Furthermore, there are additional factors, which could change the outcome of our projection. For example, in specialty crops such as berry bushes or fruit trees, which are normally grown under hail protection nets, solar modules placed above the crops can take over this protective function, leading to synergistic effects (Trommsdorf et al.,2022).
However, the impact of shading on long-term yields and qualities is still part of the research (Marrou et al., 2013).

> In order to achieve an optimal economic and ecological design and combination of energy production systems and management of agricultural land, further data collection in different systems and the knowledge gained from this are necessary.
Within the framework of our project, the operation of two systems - Agri-PV and agroforestry - is scientifically examined and compared.
The focus of this study is  the economic viability of mixed production systems, and the consideration of effects of energy production on agricultural management.
The system also considered ecosystem services on the regular crop, public acceptance and efficiency losses of the panels due to bad quality.

> Change in legislation, policy and technical innovation could lead to a promotion of the Agri-PV system. Stronger interests in ecosystem services and wood as a renewable source of energy and construction material could lead to a promotion and higher subsidies for Agroforestry systems. 
This dichotomy makes it challenging to predict which of the two systems become more viable in the future. However, combining the strengths of both systems is realistically the best option for a transition into a carbon neutral society.
